...
void malla::drawHealth(float health) {
    glBegin(GL_QUADS);
    glColor3f(1, 0, 0);
    glVertex2f(0, 0);
    glVertex2f(1, 0);
...
void malla::move_R(float k){
    this->pos = pos + glm::vec3(k, 0.0f, 0.0f);
}
void malla::jump(){
    this->velocityY += 4.5f;
}
void malla::setvelZ(float s){
    this->velocityZ = s;
}
void malla::reflect_x(){
    mat4 aux = identity_mat4();
    aux.m[0] = -1.0f;
    this->M = M*aux;
}
void malla::updatepos(float time){
    this->pos += glm::vec3(0.0f, velocityY * time, velocityZ * time);
    
    if(pos.y > 0.0f){
        this->velocityY += -13.5f * time; // -9.8f es la gravedad
    }
    else{
        this->velocityY = 0.0f;
    }
...
// sets
void malla::set_pos(glm::vec3 p){
    vec3 aux = vec3(p.x, p.y, p.z);
    this->pos = p;
    M = translate(identity_mat4(), aux);
}
